      ...
      {goals.map((goal, index) => (
        <div key={goal.id} className={`flex items-center justify-between gap-2 px-4 py-4 ${index < goals.length - 1 ? 'border-b border-slate-100' : ''}`}>
          <div className="flex items-center gap-3 min-w-0">
            {goal.completed ? (
              <span className="h-10 w-10 rounded-full bg-primary text-white flex items-center justify-center"><Check size={18} /></span>
            ) : (
              <span className="h-10 w-10 rounded-full border-[3px] border-slate-300" />
            )}
          ...
          {!goal.completed && (
            <button className="h-10 w-10 rounded-full text-[24px] leading-none text-slate-400 flex items-center justify-center" onClick={() => onSelectGoal?.(goal)}>›</button>
          )}
        </div>
      ))}
    </article>
