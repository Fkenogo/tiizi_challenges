...
function ProfileScreen() {
  ...
  const ongoingChallengesCount = analytics?.ongoingChallengesCount ?? 0;
      ...
      <div className="mx-auto max-w-mobile min-h-screen pb-[96px] bg-slate-50">
        <header className="sticky top-0 z-20 border-b border-slate-200 bg-slate-50 px-4 pt-4 pb-3">
          <div className="flex items-center justify-between">
            ...
            <button
              className="h-10 w-10 flex items-center justify-center text-slate-500 active:opacity-60"
              onClick={async () => {
                if (navigator.share) {
                  try {
                    await navigator.share({ title: 'Join me on Tiizi', text: 'Tiizi helps you build healthy habits with group challenges!', url: INSTALL_URL });
                    return;
                  } catch { /* fall through */ }
                }
                try {
            ...
            >
              <Share2 size={22} className={shareCopied ? 'text-emerald-500' : ''} />
            </button>
          </div>
        </header>
        ...
        <main className="px-4 pt-2 space-y-6">
          <section className="text-center">
            ...
            <h2 className="mt-4 st-section-title">{displayName}</h2>
            <p className="mt-2 st-body">
              Member since {memberSinceYear}
            </p>
            <button
              className="mt-3 h-9 rounded-full border border-primary px-5 text-[13px] font-bold text-primary"
          ...
          <section className="grid grid-cols-2 gap-3">
            <button className="rounded-2xl border border-slate-100 bg-white py-5 shadow-sm flex flex-col justify-center items-center" onClick={() => navigate('/app/groups')}>
            ...
            <button className="rounded-2xl border border-slate-100 bg-white py-5 shadow-sm flex flex-col justify-center items-center" onClick={() => navigate('/app/challenges')}>
              <p className="text-[16px] leading-[20px] font-black text-primary">{ongoingChallengesCount}</p>
              <p className="text-[13px] leading-[17px] tracking-[0.1em] font-bold text-slate-500 uppercase mt-1">Challenges</p>
            </button>
          </section>
          ...
          {/* Learn Tiizi guide */}
          <section
            className="rounded-2xl border border-slate-100 bg-white p-4 shadow-sm flex items-center justify-between cursor-pointer"
            onClick={() => navigate('/app/profile/learn-tiizi')}
            role="button"
            tabIndex={0}
            onKeyDown={(e) => e.key === 'Enter' && navigate('/app/profile/learn-tiizi')}
          >
            <div className="flex items-center gap-3 min-w-0">
              <div className="h-10 w-10 rounded-xl bg-primary/10 flex items-center justify-center text-[20px] leading-none flex-shrink-0">📖</div>
              <div className="min-w-0">
                <p className="text-[14px] leading-[18px] font-bold text-slate-900">Learn Tiizi</p>
                <p className="text-[12px] leading-[16px] text-slate-500 mt-0.5">Understand groups, challenges, activity logging, and how to make the most of Tiizi.</p>
              </div>
            </div>
            <span className="text-primary font-semibold text-[13px] flex-shrink-0 ml-3">Open Guide →</span>
          </section>

          <section className="rounded-2xl border border-primary/20 bg-[#fff4eb] p-5">
            ...
            <button className="mt-4 h-12 w-full rounded-2xl bg-primary text-white text-[15px] leading-[20px] font-black" onClick={() => navigate('/app/donate?trigger=manual')}>
              Keep Tiizi Free
            </button>
          </section>
          ...
          <section>
            <div className="flex items-end justify-between mb-3">
              ...
              <button className="text-[13px] font-semibold text-primary" onClick={() => navigate('/app/groups')}>See all</button>
            </div>
