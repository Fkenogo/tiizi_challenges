...
PF-05 implementation note (v1.66, status only — no governance review cycle):
PF-05 V2 Challenge Creation Wizard IMPLEMENTED on branch `impl/pf-05-v2-challenge-creation-wizard-001` (unmerged, undeployed; awaiting Founder preview and review — PF-05 is NOT marked COMPLETE). Founder-visible flow at `/app/challenges/v2/create` (V2-flag-gated; V1 `/app/create-challenge` intact): entry (scratch/template-reserved) → TYPE → BASICS → ACTIVITIES → MEASUREMENT → REQUIREMENT → SCHEDULE → RULES → REVIEW → FINISH, consuming the PF-04 Composer draft shape, Knowledge-derived options, write-nothing server preview (new read-only seams GET /v1/knowledge/:id/options + POST /v1/challenge-definitions/preview through the single PF-03 validator) and governed POST /v1/challenges establishment with post-create navigation to the V2 detail route. No V1 semantics carried forward; no Templates/Admin/UI beyond the Wizard; no migration; no deployment. 25 PF-05 guard checks + 4 API seam tests green; V2 frontend guards green; full API suite 606 passed / 8 skipped; real-PostgreSQL chain 001→017 proven; frontend typecheck clean for new files (functions/ errors pre-existing env gap); vite build clean. EBC-05 remains UNMERGED / reference-only. Master Programme 1.65 → 1.66.

PF-05-CORR-001 correction note (v1.67, status only — no governance review cycle):
PF-05-CORR-001 applied on branch `impl/pf-05-v2-challenge-creation-wizard-001` (awaiting Founder localhost preview; PF-05 remains IMPLEMENTED / NOT COMPLETE / NOT MERGED): (A) options seam accepts canonical UUID or Activity Code with single canonical-UUID frontend caching (coded-Activity path proven end to end); (B) `composerSelectable` catalogue boundary — published + V2-coded + ready + contracted candidates only (legacy/draft/retired/KCS-thin/codeless quarantined server-side); (C) Group UUID translation at the V2 seam via the identity bridge (UUID verified, legacy translated, unmapped fail closed; PG shadow never authoritative); (D) real runtime Wizard integration test (repo-standard vitest + renderer only; transport mocked, identity/cache unmocked) proving coded selection → Measurement options → establishment plus stale → refresh → options retained; plus a transport fix so structured 422 preview issues reach the Wizard (ApiError body). 25+8 guard checks, 2 runtime tests, 6 API seam tests green; full API suite 608 passed / 8 skipped; chain 001→017; typecheck/vite build clean. No Templates/Admin/deployment. EBC-05 remains UNMERGED / reference-only. Master Programme 1.66 → 1.67.
...
| ------- | ---------- | ------ | -------- | ------ |
| 1.70 | 2026-09-15 | **V1 Product Experience Freeze (TIIZI-V2-V1-FREEZE-001)** | **Founder direction — boundary only; no governance cycle** | V1 Product Experience is **FROZEN / REFERENCE ONLY**. PF-05 technical/domain implementation remains **IMPLEMENTED**, but its experience assembly is **NOT APPROVED**: Founder component review stopped after V1 experience composition was identified. A static V2→V1 experience import boundary records existing PF-05 `BottomNav` contamination and rejects new frozen dependencies; governed V2 Product Truth, Group authority, Auth, PF-01→PF-04, Engines, preview harness, and V2 establishment/read model remain intact. Greenfield Experience Reference is **IN DEVELOPMENT / NOT YET ADOPTED**; no broad V2 Experience Assembly precedes its Founder review. **PF-05: NOT COMPLETE / NOT MERGED. PF-06: NOT BEGUN.** No V1 deletion, domain change, deployment, or merge. |
| 1.69 | 2026-09-15 | **PF-05 Challenge Creation Component Preview (TIIZI-PF05-CHALLENGE-CREATION-COMPONENT-PREVIEW-001)** | **Implementation status only — no governance cycle** | PF-05 Challenge Creation Component: **IMPLEMENTED / AWAITING FOUNDER COMPONENT REVIEW**. A strict local-development-only component route composes the existing PF-05 Wizard with a deterministic governed preview Group: Firestore Group and live owner Membership remain authoritative; PG is shadow/identity only; the normal bridge, PF-04 Composer, PF-03 validation, V2 establishment, and V2 read model remain in use. The neutral result view never enters V1 navigation. No V2 application shell, Group UI repair, Challenge seed, Templates, merge, deployment, or PF-06 work. **PF-05: NOT COMPLETE / NOT MERGED. PF-06: NOT BEGUN.** |
| 1.68 | 2026-09-15 | **PF-05 local Founder preview harness (TIIZI-PF05-LOCAL-PREVIEW-HARNESS-001)** | **Implementation status only — no governance cycle** | PF-05 local Founder preview harness implemented / awaiting Founder preview on `impl/pf-05-v2-challenge-creation-wizard-001`: explicit local Auth (9099), Firestore (8080), Emulator UI (4401) and API (4000) configuration; flag-gated browser emulator wiring; emulator-only synthetic Auth seed; localhost-only dry-run/apply Member mappings; existing governed Group creation preserves live Firestore authority with PG shadow synchronization. No Challenge seed, no Wizard semantic/product-flow change, no deployment, no merge. PF-05 remains IMPLEMENTED / NOT COMPLETE / NOT MERGED. PF-06 remains not begun. |
| 1.67 | 2026-09-14 | **PF-05-CORR-001 wizard runtime correction (TIIZI-V2-PF-05-CORR-001)** | **Settled product semantics only (correction; no governance cycle)** | Records PF-05-CORR-001 applied on branch `impl/pf-05-v2-challenge-creation-wizard-001` (awaiting Founder localhost preview; PF-05 NOT COMPLETE/NOT MERGED): UUID/Code options identity, composer-selectable catalogue boundary, Group UUID translation seam, runtime Wizard integration test, structured preview-issue transport. Guards + 2 runtime + 6 seam tests green; full API suite 608 passed / 8 skipped; chain 001→017; typecheck/vite build clean. No Templates/Admin/deployment. EBC-05 remains UNMERGED / reference-only. Master Programme 1.66 → 1.67. |
| 1.66 | 2026-09-14 | **PF-05 V2 Challenge Creation Wizard implemented (TIIZI-V2-PF-05-V2-CHALLENGE-CREATION-WIZARD-001)** | **Settled product semantics only (implementation; no governance cycle)** | Records PF-05 IMPLEMENTED on branch `impl/pf-05-v2-challenge-creation-wizard-001` (unmerged, undeployed; awaiting Founder preview/review — NOT marked COMPLETE): Wizard UI + read-only preview/options seams, 25 guard checks + 4 seam tests green, full API suite 606 passed / 8 skipped, real-PG chain 001→017, vite build clean. No Templates/Admin/deployment. EBC-05 remains UNMERGED / reference-only. Master Programme 1.65 → 1.66. |
