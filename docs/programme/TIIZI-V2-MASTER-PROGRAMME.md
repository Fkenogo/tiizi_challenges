...
| ------- | ---------- | ------ | -------- | ------ |
| 1.72 | 2026-09-16 | **S2b — V2 Group Context + Challenge Creation IMPLEMENTED CANDIDATE / AWAITING FOUNDER PRODUCT PREVIEW (TIIZI-S2B-IMPL-001)** | **Authorised implementation slice (not a governance cycle)** | Records **S2 IN PROGRESS** and **S2b IMPLEMENTED CANDIDATE / AWAITING FOUNDER PRODUCT PREVIEW** on branch `impl/s2b-v2-challenge-creation-001` from exact `origin/main` @ `b97fbf618dce864206594db61d29680e86c5382c`; **S2 is NOT marked COMPLETE**; S2a/S1/EA-01 remain closed. S2b is the first **vertical product assembly** binding the adopted six-step Challenge-creation experience to already-merged governed capability: V2 Challenges entry/read (`GET /v1/challenges`; loading/empty/populated/created-visible), minimal host-Group context from the member's real `GET /v1/memberships/me` (no global active-Group, no V1 Groups prerequisite, no standalone Challenge), composer-selectable catalogue (`GET /v1/knowledge?composerSelectable=true`), governed options with progressive disclosure (`GET /v1/knowledge/:id/options`; Metric/Unit/Components/Load basis/Duration mode/Completion occurrence), human-facing Together/Race/Streak mapped to canonical `collective`/`competitive`/`streak`, schedule with friendly timezone labels and derived inclusive window, explicit creator participation (never auto-enrolled; `join_creator`), server preview/validation (`POST /v1/challenge-definitions/preview`; no second validator, no client-side semantic duplication), governed establishment (`POST /v1/challenges` + `ChallengeCreationAuthority`; no Firestore/PG client write; idempotent retry; governed denials translated), and a persisted created-Challenge context (`GET /v1/challenges/:id`) that survives refresh. Additive server changes: derived `unitsByMetric` on the S2a options seam and `metric`/`requiredComponents`/`loadReportingBasis`/`durationMode`/`completionOccurrence` on the V2 detail read contract. Local preview harness `npm run preview:s2b:seed` (loopback-only, refuses production; Auth+Firestore emulators, migrations 001–017, deterministic member/Group/membership/Knowledge; no Challenges seeded). Experience boundary extended: `src/features/Challenges/**` wholesale import-forbidden, PF-05 UI identifiers prohibited, no direct Firestore write in `src/v2/challenges/**`. Tests: API suite incl. `s2bChallengeCreation.test.ts`; TypeScript; build; `test:v2-experience-boundary`, `test:s2a-challenge-creation-api-seam`, `test:s2b-challenge-creation`, `test:v2-frontend`, `test:v2-auth-returns`, `test:firebase-emulator-mode`, `test:preview-v2-auth-workflow`; local end-to-end loopback smoke (sign-in → catalogue → options → preview → establish Together/Race/Streak → list → detail). **PF-05 NOT merged/cherry-picked; PF-06 NOT begun; no Templates/Support/Cause/Recognition; no deployment; no production mutation. STOP BEFORE MERGE.** Master Programme 1.71 → 1.72. |
