...
| ------- | ---------- | ------ | -------- | ------ |
| 2.04 | 2026-09-23 | **S4 ACTIVE; S4a IMPLEMENTED CANDIDATE / TECHNICALLY REVALIDATED / AWAITING FOUNDER ACCEPTANCE (TIIZI-S4A-INDEPENDENT-TECHNICAL-REVALIDATION-AND-FOUNDER-PREVIEW-001)** | **Revalidation record (documentation only; PR #46 NOT merged; no deploy; no S4b/c/d)** | Records independent revalidation of the S4a candidate (PR #46, head `303bc9a`, base `origin/main` @ `a48c106`, verified no drift; repo `ci` green on the candidate) with disposition **B — PASS WITH NON-BLOCKING OBSERVATIONS**: (1) no second Group authority/store — PG shadow maps identity only, live Firestore doc controls existence/liveness/settings/count/stewardship, viewer relationship server-derived, steward singular and server-resolved, private/inactive/unknown reads fail closed with no existence oracle and no UID/legacy/invite leaks (field-by-field source classification recorded in the revalidation report); (2) `GET /v1/challenges?groupId=` narrows the entitled set server-side — members see genuinely scoped Challenges, private outsiders see nothing, public outsiders see summary-only discovery (detail/leaderboard/contributors remain 404), unfiltered list unchanged; non-blocking observation: `establishment`-status (scheduled/unactivated) Challenges appear in public discovery summaries — chartered EOG §9 subset, recommend status-gating when S4c discovery lands; (3) creation exposes only Identity + three governed Community Setup choices with authority-matching defaults, navigates to the canonical returned Group ID with fresh server read; (4) Group Home presents identity/purpose/relationship/steward/count/settings/hosted Challenges with loading/error/not-found/empty states and member-worded copy, no raw identifiers; non-blocking observation: legacy `admin`-role rows render the S2-G "Accountable Steward" label — presentation-only, reconcile in S4b; (5) hosted rows reuse existing Challenge presentation and routes; full wizard loop closed in a real browser (host preselected, ESTABLISH 201, genuinely scoped on Home); forged/outsider/restricted establishment rejected by the real creation authority with zero forged rows; (6) real-browser pass 6/6 widths (320/375/390/430/1024/1440, zero overflow, zero console/page errors) plus remote-equivalent public-tunnel journey (sign-in, seeded groups, create→Home, return+refresh) with zero localhost traffic and zero failed responses. Regression: API suite 698–704 passed / 8 skipped; all S2-G/S2b/S3/V2/boundary/nav guards green; typechecks + api/web/functions builds green; `test:challenge-creation-6combos` failure proven pre-existing/environmental (identical signature on unmodified post-charter main — needs live Firestore exercise data). **S4 remains ACTIVE (not complete); S4a NOT Founder-accepted, NOT complete, NOT merged; S1–S3 remain closed; Stage F/CGP not reopened.** Next: Founder preview/acceptance of PR #46. Master Programme 2.03 → 2.04. |
| 2.03 | 2026-09-23 | **S4 ACTIVE; S4 CHARTERED (TIIZI-S4-GROUPS-EXPERIENCE-CHARTER-001; implementation NOT STARTED / NOT AUTHORISED pending Founder acceptance)** | **Charter (documentation + implementation authorisation only; STOP BEFORE MERGE; no schema, no migration, no API/UI implementation, no deploy)** | Records **S4 ACTIVE** on candidate branch `docs/s4-groups-experience-charter-001` (base `origin/main` @ `561df38`, verified no drift from the S3-closure baseline). Files `docs/experience/TIIZI-S4-GROUPS-EXPERIENCE-CHARTER.md` (new): reconciles adopted Experience Reference Group surfaces against Product Truth (ADOPT landing/detail-header/creation composition/governed toggles/join journey; ADAPT counts/members/admins/invite-code format/Charter presentation; REJECT cross-Challenge leaderboard, media pipeline, location; DEFER Group Feed — no publication authority exists — Feed must not block S4a). Authorises Founder-directed sequence **S4a (Comprehensive Group Creation + Group Home) → S4b (Members + Stewardship) → S4c (Discovery + Join + Invitations) → S4d (Settings + Governed Configuration)**. S4a reuses `POST /v1/groups` (full governed field set already accepted), join/leave, `GET /v1/memberships/me`; adds ONLY `GET /v1/groups/:groupId` (live-authority detail, visibility-gated, fail-closed) and a governed `groupId` filter on `GET /v1/challenges`. **S3 remains COMPLETE / FOUNDER ACCEPTED / MERGED (not reopened); S1/S2 unchanged; Stage F/CGP not reopened; S4 NOT complete; S4a NOT implemented.** Next authorised action: S4a implementation on Founder charter acceptance. S5–S9 remain downstream. Master Programme 2.02 → 2.03. |
| 2.02 | 2026-09-22 | **S3 IMPLEMENTATION IN PROGRESS; S3d COMPLETE / FOUNDER ACCEPTED / MERGED (TIIZI-S3D-MERGE-AND-CLOSURE-001)** | **Founder-approved merge + closure (normal merge-commit path; no squash, no rebase, no force-push; no deploy)** | Records **S3d COMPLETE / FOUNDER ACCEPTED / MERGED**. PR #42 (`impl/s3d-results-finalized-experience-001`) was merged into canonical `origin/main` by normal merge commit `39833648be303b77617526cf3eb6d385dc4bdc92` of reviewed source `3f26ec6349b522cd7049fd2144fa36dde6dd47e6` (pre-merge main `f98a71bf0ba3db9e932571ffaee4bd88a5da5f6e`; reviewed source verified ancestor of main). PR #42 gating `ci` green on the reviewed source (api, api-image, functions, web); external `Workers Builds: tiizi-challenges` failure is NOT an S3 gate per FD-S3-005 (fails identically on every recent `origin/main` commit; no Cloudflare remediation performed). The bounded mobile-navigation correction (TIIZI-MOBILE-PRIMARY-NAV-CORR-001/CORR-002) was reconciled: original reviewed head `479dbdef58119da9f1d0ec46480854bd73ee1ca7`, rebased onto post-#42 main `3983364` (clean rebase — the only shared file `V2ChallengeListScreen.tsx` auto-merged, no conflict), reconciled head `3feb08a76dbd9ecb50755522d9d797545333482b`, merged by normal merge commit `6f6375bf883716b22e5294b770c7556864943d15`; both accepted histories verified ancestors of final `origin/main`. Combined behaviour retained: pending list uses the governed end state and reads Finished (never raw Running); Activity Guide contextual link under Create Challenge and in the creation step; mobile primary nav exactly Today / Challenges / Groups; Profile/Notifications in the header; emulator warning disabled for the preview; no S3d result behaviour change. **Post-merge validation on final `origin/main`:** API suite 685 passed / 8 skipped; root `tsc -b`, `vite build`, functions build clean; `test:s3d-results`, mobile-primary-nav, firebase-emulator-mode, S3c/S3b/S3a and V2 frontend/boundary guards green; `git diff --check` clean; no schema/migration/deployment change introduced during reconciliation. **S3a/S3b/S3c remain COMPLETE / FOUNDER ACCEPTED / MERGED (S3c NOT reopened); no new S3d scope; no deployment authorization implied by this closure.** Master Programme 2.01 → 2.02. |
| 2.01 | 2026-09-22 | **S3 IMPLEMENTATION IN PROGRESS; S3d RESULTS / FINALIZED EXPERIENCE IMPLEMENTED CANDIDATE / CORRECTED / AWAITING FOUNDER ACCEPTANCE (TIIZI-S3D-RESULTS-EXPERIENCE-CORR-002)** | **Bounded presentation-only correction on PR #42 (STOP BEFORE MERGE; acceptance is a separate Founder disposition)** | Applies one bounded Founder-preview presentation correction on branch `impl/s3d-results-finalized-experience-001` (entry re-verified: `origin/main` @ `f98a71bf0ba3db9e932571ffaee4bd88a5da5f6e` is ancestor of the PR head, no drift; entering head `7088c921e661a0391db03bdb59f213f014528116`; PR #42 OPEN / NOT MERGED). **Presentation only — no Challenge semantics, finalization, frozen truth, lifecycle authority, Race member identity, database schema, migrations, activity acceptance, ranking, recognition or result derivation changed; CORR-001 authority/lifecycle/sealed-result corrections preserved.** (A) **Outcome-first finalized hierarchy** via pure helpers (`collectiveOutcomeFor` / `competitiveOutcomeFor` / `streakOutcomeFor`): Together reached leads with "Goal reached" and keeps actual total/goal plus uncapped percent and overshoot (missed uses neutral "{actual} of {goal} {unit} completed"); Race positioned leads with "You finished #{position}" and unfinished with "You reached {progress} of {target} {unit}" plus "Progress at close", keeping governed progress and Final standings with server-projected positions authoritative and no winner/podium/medal/recognition/client ranking; Streak reached leads with "You reached the {requiredDays}-day streak" while Best streak, Final streak, Days completed, required run and day-by-day history remain supporting facts (frozen `final_streak` still rendered, including 0, never `currentStreak`; not reached uses neutral factual wording). (B) **Finalized "Taking part" card removed** on finalized results only (no participant-facing result value); underlying participation/lifecycle authority unchanged — Join, Leave and Log Activity remain unavailable after finalization; live/ended participation UI unchanged. (C) **Ended-pending hero status** derived from the existing server-governed `endStateFor` path (`statusLabelForEndState`): a window-expired-but-unprocessed Challenge no longer reads "Running" (reads "Finished"); no device clock, no domain status added, no status mutated for presentation; pending card still reads "Challenge ended / Final results are being confirmed". Evidence: `scripts/testS3dResultsGuards.ts` gains 13 CORR-002 checks covering the twelve required proofs plus re-asserted CORR-001 fail-closed discriminators; API full suite 685 passed / 8 skipped; S3d/CORR-001/PR #41 Race identity/S3c/S3b/S3a/S2b/membership/frontend/boundary guards green; API typecheck/build, root `tsc -b`, `vite build`, functions build clean; `git diff --check` clean. Record: `docs/experience/TIIZI-S3D-RESULTS-FINALIZED-EXPERIENCE.md` §11. **S3d remains IMPLEMENTED CANDIDATE / CORRECTED / AWAITING FOUNDER ACCEPTANCE — NOT COMPLETE, NOT FOUNDER ACCEPTED, NOT MERGED; PR open; no merge, no deploy. S3c is NOT reopened (COMPLETE / FOUNDER ACCEPTED / MERGED); S3a/S3b unchanged; S3 remains IMPLEMENTATION IN PROGRESS.** Master Programme 2.00 → 2.01. |
| 2.00 | 2026-09-22 | **S3 IMPLEMENTATION IN PROGRESS; S3d RESULTS / FINALIZED EXPERIENCE IMPLEMENTED CANDIDATE / CORRECTED / AWAITING TECHNICAL REVALIDATION (TIIZI-S3D-RESULTS-FINALIZED-EXPERIENCE-CORR-001)** | **Bounded correction on PR #42 (STOP BEFORE MERGE; acceptance is a separate Founder disposition)** | Corrects the three blocking findings of TIIZI-S3D-RESULTS-FINALIZED-EXPERIENCE-ITR-001 (disposition **C — CORRECTION REQUIRED BEFORE FOUNDER PREVIEW**) on branch `impl/s3d-results-finalized-experience-001` (entry re-verified: no drift; base `origin/main` @ `f98a71bf0ba3db9e932571ffaee4bd88a5da5f6e`; entering head `0dc9eb9ed7842d9e9c7ac72ed7f5abab7ef392ef`; PR #42 OPEN / MERGEABLE): **(A) server-authoritative join lifecycle** — `joinChallenge` now refuses ended, finalized, or active-status window-expired Challenges through one shared `requireParticipationMutable` authority (same `getGoverningVersion` + `dayInTimezone` seam as activity acceptance; never device time; governed `422 challenge_ended`; no new lifecycle status; no episode created); **(B) server-authoritative withdrawal lifecycle** — `withdrawParticipation` runs the same authority before any write, so window-expired / ended / finalized withdrawal is rejected and the participation episode is unchanged; **(C) finalized-result source rule** — a FINALIZED read now sources Challenge-level result fields from `challenge_finalizations` (class F) and per-participation result fields from immutable governed evidence reconstructed through the authorised engine fold `recomputeChallengeDerived` (class I), never the mutable derived projection (class L); contributor totals are anchored to the sealed total with reconciliation enforced (fail closed on divergence); a finalized marker without stored finalization fails closed; the client `resultsView.ts` no longer falls back from absent frozen fields to live totals/completion/position/streak (finalized components render nothing as a final result). Field-level **F/I/L trace** recorded in `docs/experience/TIIZI-S3D-RESULTS-FINALIZED-EXPERIENCE.md` §8. Participant-facing copy "Final positions come from the Challenge server" removed. Evidence: new `api/test/s3dResultsFinalizedCorr001.test.ts` (11 production-seam tests A/B/F/G/H/I; **9 of 11 proven to fail against the pre-correction PR #42 implementation**, the 2 passing cases being the unchanged-behaviour controls) plus CORR-001 fail-closed guards; existing participation/foundation fixtures pin the governed clock inside their 2026-06 windows without weakening any assertion. **S3d is NOT COMPLETE, NOT FOUNDER ACCEPTED and NOT MERGED; PR #42 remains open; no merge, no deploy, no Founder preview, no new S3d scope.** S3c is NOT reopened (COMPLETE / FOUNDER ACCEPTED / MERGED); S3a/S3b unchanged; S3 remains IMPLEMENTATION IN PROGRESS. No schema, no migration. Record: `docs/experience/TIIZI-S3D-RESULTS-FINALIZED-EXPERIENCE.md`. Master Programme 1.99 → 2.00. |
| 1.99 | 2026-09-21 | **S3 IMPLEMENTATION IN PROGRESS; S3d RESULTS / FINALIZED EXPERIENCE IMPLEMENTED CANDIDATE / AWAITING TECHNICAL REVIEW (TIIZI-S3D-RESULTS-FINALIZED-EXPERIENCE-001)** | **Authorised implementation slice (STOP BEFORE MERGE; acceptance is a separate Founder disposition)** | Records S3d **IMPLEMENTED CANDIDATE / AWAITING TECHNICAL REVIEW** on branch `impl/s3d-results-finalized-experience-001` (base `origin/main` @ `f98a71bf0ba3db9e932571ffaee4bd88a5da5f6e`; entry re-verified: no drift; Master Programme v1.98). Implements the bounded, Founder-authorised S3d scope with authority order preserved (engine/frozen truth → persisted finalization → read model → client contract → type-specific experience): (A) **honest lifecycle states** — live (existing S3c surfaces unchanged), window-expired-unprocessed and ended-not-finalized (neutral read-only "Challenge ended / Final results are being confirmed", logging and Leave unavailable), finalized (sealed type-specific results); time derived only from server-projected `governingToday`/`endDate`, never the device clock; (B) **bounded read projection** — `GET /v1/challenges/:id` (and list) now exposes a frozen per-participation `final` block (`completed`, `completedAt`, `daysCompleted`, `bestStreak`, `finalStreak`, `finalPosition`, `finalizedAt`) projected read-only from `challenge_participation_finals`, null while unfinalized, governing (earliest completed) episode for Race per PR #41; `finalized`/`finalizedAt`/`finalResult`/`finalPosition` typed on the V2 client; **no schema, no migration, no second results authority, no frontend recomputation**; frozen `finalStreak` is exposed per FD-S3D-2A and the live `currentStreak` is never substituted; (C) **experience** — Together final group total vs goal with overshoot preserved (220/200 = 110%), reached/actual result (no failure wording), own contribution + member-level contributors (no ranking); Race final result with served frozen `Final position` (ties share) and **Final standings** (finishers then participants at close with actual progress, N of M), no winner/podium; Streak personal result (days completed of the full period, best streak, **Final streak**, required run, per-day history), no leaderboard; restrained "Results saved on …" copy with no permanence promise (ACT-04 deferral preserved); no backend terminology in member copy. Evidence: new real-seam `api/test/s3dResultsFinalizedExperience.test.ts` (T-1, T-2, T-3, T-4 incl. HTTP `challenge_not_active`, T-5 ties 1,1,3 + non-finisher null, T-8 no partial exposure, PR #41 member-identity result; T-6/T-7 delivered by PR #41) — API full suite 674 passed / 8 skipped; new `test:s3d-results` guards (G-1 pure derivation, G-2 end-state routing, G-3 finalized-standings lifecycle through a real QueryClient, G-4 S3c boundary preserved incl. live-surface unmount when finalized, G-5 no failure/winner/podium/recognition vocabulary, no manufactured truth, no V1 imports); S3c/S3b/S3a/S2b/membership/frontend/boundary guards green; API typecheck/build, root `tsc -b`, `vite build`, functions build clean; `git diff --check` clean. **S3d is NOT COMPLETE, NOT FOUNDER ACCEPTED and NOT MERGED; PR open; no merge, no deploy.** S3c is NOT reopened (COMPLETE / FOUNDER ACCEPTED / MERGED); S3a/S3b unchanged; S3 remains IMPLEMENTATION IN PROGRESS. Explicitly out of scope and untouched: scheduler / automatic finalization, finalization HTTP mutation endpoint, operator finalization UI, Run Again, recognition issuance, badges/rewards, Kudos, Support Tiizi / donations / Cause, media, S4/S8/S9, ACT-03/ACT-04 correction mechanism, schema redesign, migrations. Record: `docs/experience/TIIZI-S3D-RESULTS-FINALIZED-EXPERIENCE.md`. Master Programme 1.98 → 1.99. |
| 1.98 | 2026-09-21 | **S3 IMPLEMENTATION IN PROGRESS; S3c COMPLETE / FOUNDER ACCEPTED / MERGED; S3d READINESS CLOSED; RACE MEMBER-IDENTITY BLOCKER CLOSED; S3d READY FOR IMPLEMENTATION / NOT STARTED (TIIZI-S3D-RACE-MEMBER-IDENTITY-CLOSE-001)** | **Closure record (documentation only; no S3d implementation, no deploy)** | Records TIIZI-S3D-RACE-MEMBER-IDENTITY-CLOSE-001: PR #41 (`fix/s3d-race-member-identity-001`) MERGED by normal merge commit `0fe1fb5f65b1c5d3adaddd5f733906d3b522f576` of accepted final head `b9f8ab8ff99a5d9ea3c544e7091887ffa8360271` (production implementation unchanged from independently reviewed `f641e12`; only ITR-001 regression hardening added, `api/test/s3dRaceMemberIdentity.test.ts` 12 to 14; merge-base equalled pre-merge main `494ee23d40d37b97a8e11fac16535a427b1ac270`, no drift; accepted head verified ancestor of main; repo `ci` green on the final head — api, api-image, functions, web; Workers Builds non-gating per FD-S3-005). PR #40 reconciled: readiness assessment sections 1-13 preserved; disposition/closure material records the independently verified Race member-identity issue corrected by PR #41 (member-scoped competitive identity; progress/evidence participation-episode scoped; unfinished progress does not accumulate across leave/rejoin; already-completed member remains completed on the earliest completed episode; `ebc04/v2` on new member-scoped finalization semantics with historical `ebc04/v1` results verified under V1 semantics; Race blocker CLOSED). **S3c remains COMPLETE / FOUNDER ACCEPTED / MERGED (not reopened); S3a/S3b unchanged; S3 remains IMPLEMENTATION IN PROGRESS; S3d READY FOR IMPLEMENTATION but NOT STARTED.** Next authorised programme action is bounded S3d implementation only (neutral ended/not-yet-finalized state; frozen Streak projection incl. `final_streak`/`final` block; finalized results presentation for Together/Race/Streak; required client typing; remaining approved S3d regression/guard coverage). No scheduler. No Run Again. No recognition issuance. No Kudos/Support. No deployment. Master Programme 1.97 to 1.98. |
| 1.97 | 2026-09-21 | **S3 IMPLEMENTATION IN PROGRESS; S3d READINESS ASSESSED / FOUNDER DISPOSITION RECORDED; S3d NOT STARTED (TIIZI-S3D-PREIMPLEMENTATION-READINESS-001 / TIIZI-S3D-READINESS-DISPOSITION-001)** | **Assessment acceptance + Founder decisions (documentation only; no S3d implementation, no deploy)** | Records Founder acceptance of the S3d pre-implementation readiness assessment (`docs/programme/TIIZI-S3D-PREIMPLEMENTATION-READINESS-001.md`, PR #40; reviewed main `494ee23`; disposition D — MIXED) and its Founder decisions: **FD-S3D-1** approved (ended-not-yet-finalized Challenges show a neutral read-only "Challenge ended — Final results are being confirmed." state; provisional results are never presented as final; sealed results render only once canonical finalization exists); **FD-S3D-2A** approved (existing frozen Streak final truth, incl. `final_streak` and the frozen per-participation final block, may be exposed through the V2 read model — projection of existing authority, no new domain behaviour, no schema/migration; live `currentStreak` must not stand in for frozen Final Streak); **FD-S3D-3** approved (participant vocabulary — Together: Final result / Group total / Your contribution; Race: Final results / Final standings / Final position / Finished; Streak: Final result / Best streak / Days completed / Final streak; transitional: Challenge ended / Final results are being confirmed; Winner, Podium, Success, Failure, recognition tiers and badges/rewards NOT authorised). **Founder product rule (Race):** a member is ONE competitive participant in a Race Challenge; leave/rejoin must not create multiple competitive identities in final Race results and historical participation episodes remain preserved. The RACE-EDGE is reproduced, root-caused (competitive identity was the participation episode in completion/finalization/count, the read model was member-based) and corrected as a bounded API/domain fix (classification C: finalization aggregation + read model; no schema, no migration, no evidence rewrite; `ebc04/v2` stamp with version-gated verification so `ebc04/v1` frozen history still verifies) on PR #41 (`fix/s3d-race-member-identity-001`, MERGED TIIZI-S3D-RACE-MEMBER-IDENTITY-CLOSE-001). **S3d remains NOT STARTED:** the results UI, the Streak projection, transitional state, client final-result typing and all other S3d experience are authorised direction for the NEXT implementation task. The PR #41 (Race identity) gate is CLOSED. **S3c is NOT reopened (COMPLETE / FOUNDER ACCEPTED / MERGED); S3a/S3b unchanged; S3 remains IMPLEMENTATION IN PROGRESS.** No deployment, no migration, no production mutation. Record: assessment §14. Master Programme 1.96 → 1.97. |
| 1.96 | 2026-09-21 | **S3 IMPLEMENTATION IN PROGRESS; S3c stays COMPLETE / FOUNDER ACCEPTED / MERGED — bounded post-S3c presentation cleanup (TIIZI-S3C-DETAIL-CLEANUP-001)** | **Backlog-style post-acceptance experience cleanup (merged; PR #39; normal merge commit; no deploy, no S3d)** | Records a bounded presentation cleanup identified during continued Founder visual review **after** S3c acceptance/merge: the standalone "What counts" card is removed from the joined Challenge detail for Together/Race/Streak, because the participant surfaces already express it (Group Progress + shared target + contributors; Race Progress + target + Race Standings; Today's Daily Consistency + requirements/streak/days) and the governed Log Activity overlay exposes governed activity names/units at the point of logging. The Streak duration rule is preserved once inside Today's Daily Consistency from the governed `requiredConsecutiveDays`. Presentation only — no canonical Challenge truth, contract, read projection, logging eligibility, activity identity, target, unit, progress calculation, race ranking, streak or participation semantics changed; Log Activity overlay and Leave Challenge flow untouched; no migration. **S3c is NOT reopened and its acceptance is unchanged (COMPLETE / FOUNDER ACCEPTED / MERGED); S3a/S3b remain COMPLETE / FOUNDER ACCEPTED / MERGED; S3 remains IMPLEMENTATION IN PROGRESS; S3d NOT STARTED.** Record: `docs/experience/TIIZI-S3C-LIVE-PROGRESS.md` §16. Master Programme 1.95 → 1.96. [MERGED 2026-09-21: PR #39 merged by normal merge commit `e9cec23cb4d610db5f594420ddfb464297254f61` of accepted head `6ae21d72cb1ea229c64ca80ab9ebe080e32d96b7`; accepted head verified ancestor of main; pre-merge main `43720f7` (no drift); repo `ci` green on the accepted head and on the merge commit. S3c NOT reopened.] |
| 1.95 | 2026-09-21 | **S3 IMPLEMENTATION IN PROGRESS; S3c COMPLETE / FOUNDER ACCEPTED / MERGED (TIIZI-S3C-FOUNDER-ACCEPT-MERGE-001)** | **Merged-state record (documentation only; no deploy, no S3d)** | Records **S3c COMPLETE / FOUNDER ACCEPTED / MERGED**. PR #38 (`impl/s3c-live-progress-001`) was merged into canonical `origin/main` by **normal merge commit** `144e516fe2be6ce6142442acf587ac282e5a0afa` (no squash, no rebase, no force-push) of accepted head `2286b6a7eb065724c4d324d3b0a59cb2182a2d22`; the accepted head is verified an **ancestor of `origin/main`**. Pre-merge main was `af1d67ef8797cc887f4de09a178ae6af2a6d4a37`; merge-base equalled pre-merge main (no drift). PR metadata reconciled before merge (title/body no longer described the candidate as awaiting review). Pre-merge repository `ci` on the exact accepted head green (api, api-image, functions, web); post-merge `ci` on `144e516` green (run `35583192641`); external `Workers Builds: tiizi-challenges` failure is NOT an S3 gate per FD-S3-005. Founder acceptance retained: Together/Race/Streak type-specific progress truth; hero hierarchy; Log Activity overlay; activity display names; Leave Challenge confirmation; CSS hero fallback accepted for the current slice; canonical media still deferred behind its authorised contract. **S3a/S3b remain COMPLETE / FOUNDER ACCEPTED / MERGED; S3 remains IMPLEMENTATION IN PROGRESS; S3d NOT STARTED.** Master Programme 1.94 → 1.95. |
| 1.94 | 2026-09-21 | **S3 IMPLEMENTATION IN PROGRESS; S3c COMPLETE / FOUNDER ACCEPTED / READY TO MERGE (TIIZI-S3C-FOUNDER-ACCEPT-MERGE-001)** | **Founder acceptance + authorised merge (normal merge-commit path; no squash, no rebase, no force-push; no deploy)** | Records **S3c COMPLETE / FOUNDER ACCEPTED** on branch `impl/s3c-live-progress-001` (accepted head `2286b6a`; PR #38 OPEN; entry re-verified: merge-base equals `origin/main` @ `af1d67e`, no drift; repo `ci` on the exact accepted head green — api, api-image, functions, web; external `Workers Builds: tiizi-challenges` failure NOT a gate per FD-S3-005). **Founder acceptance evidence:** localhost preview of the accepted candidate proved the Together / Race / Streak live-progress journeys with the common hero, Log Activity CTA + overlay, governed activity names, participant language, Create Another placement, and Leave Challenge as a secondary hero action with confirmation disclosure and mutation-free cancellation; CORR-003 was the final bounded correction. Slice record: `docs/experience/TIIZI-S3C-LIVE-PROGRESS.md` §15. Accepted points retained: type-specific progress truth; hero hierarchy; Log Activity overlay; activity display names; Leave Challenge confirmation; CSS hero fallback accepted for the current slice; canonical media still deferred behind its authorised contract. **S3a/S3b remain COMPLETE / FOUNDER ACCEPTED / MERGED** (unchanged); **S3 remains IMPLEMENTATION IN PROGRESS; S3d NOT STARTED.** Validation: CORR-003 + S3c/S3b-CORR-001/S3a/S2b/membership/frontend/boundary guards green; API full suite 652 passed / 8 skipped; API typecheck/build, root `tsc -b`, `vite build`, functions build clean; `git diff --check` clean. Merge authorised: PR #38 by NORMAL MERGE COMMIT (no squash/rebase/force-push). Master Programme 1.93 → 1.94. |
| 1.93 | 2026-09-21 | **S3 IMPLEMENTATION IN PROGRESS; S3c IMPLEMENTED CANDIDATE / CORRECTED / TECHNICALLY REVALIDATED / EXPERIENCE ALIGNMENT CORRECTED / LEAVE INTERACTION CORRECTED / AWAITING FOUNDER ACCEPTANCE (TIIZI-S3C-FOUNDER-PREVIEW-CORR-003)** | **Bounded Founder-preview experience correction on PR #38 (STOP BEFORE MERGE)** | Simplifies the Leave Challenge interaction on branch `impl/s3c-live-progress-001` (base `origin/main` @ `af1d67e`; entry re-verified: no drift; entering head `5453496`; PR #38 OPEN / MERGEABLE) with no new product semantics and no engine/domain change: the permanent TAKING PART card is removed for active participants; the common hero carries a visually secondary Leave Challenge action subordinate to the primary + Log activity CTA (all three types; no explanatory history text in the hero); selection opens a `V2Sheet` confirmation disclosing that leaving ends the current participation while Challenge history is kept, with Stay in Challenge / Leave Challenge actions; only explicit confirmation executes the existing governed `POST /v1/challenges/:id/withdraw` path with canonical invalidation/refetch (post-leave truth re-derives to the existing Join-again state). New CORR-003 guards prove hero Leave visibility, card absence, open-only first click, mutation-free cancellation, exactly-once governed leave, and intact Log Activity/progress surfaces. Evidence: CORR-003 guards PASS; S3c/S3b-CORR-001/S3a/S2b/membership/frontend/boundary guards PASS; api full suite 652 passed / 8 skipped (incl. participation + S3c routes); API typecheck/build, root `tsc -b`, `vite build`, functions build clean; `git diff --check` clean; localhost Founder preview re-verified on all three challenges (dialog opens, cancel preserves, isolated confirmed-leave proof on scratch state). Founder accepted the CORR-002 hierarchy, differentiation, hero, Log Activity CTA/overlay, activity names and CSS fallback; final S3c acceptance remains with Founder after this preview. **S3c is NOT marked COMPLETE or FOUNDER ACCEPTED; PR #38 remains unmerged; no deploy; S3d NOT STARTED.** Master Programme 1.92 → 1.93. |
| 1.92 | 2026-09-21 | **S3 IMPLEMENTATION IN PROGRESS; S3c IMPLEMENTED CANDIDATE / CORRECTED / TECHNICALLY REVALIDATED / FOUNDER PREVIEW FUNCTIONALLY PASSED / EXPERIENCE ALIGNMENT CORRECTED / AWAITING FOUNDER ACCEPTANCE (TIIZI-S3C-FOUNDER-PREVIEW-CORR-002)** | **Founder-preview experience-alignment correction on PR #38 (STOP BEFORE MERGE)** | Reassembles the S3c candidate on branch `impl/s3c-live-progress-001` (base `origin/main` @ `af1d67e`; entry re-verified: no drift; entering head `e28438d`; PR #38 OPEN / MERGEABLE) around the adopted Experience Reference hierarchy with canonical S3c truth unchanged (no engine/domain/cache/finalization change): common detail shell (Back → hero → concise context → type live progress → supporting info); Experience-Reference hero for Together/Race/Streak (type badge, title, host Group, schedule/timezone, purpose, Log Activity CTA; bounded CSS fallback — the V2 challenge contract exposes no governed media field and cover media remains blocked on an authorised media contract, so no image is fabricated); Log Activity as CTA opening the existing governed interaction in the `V2Sheet` overlay (same activities/identity/units/validation/timestamp/API path/success/invalidation; accepted feedback + Done; refetch-only convergence); participant language (Group progress, Race progress/Race standings, Today's Daily Consistency, Current/Best streak, Days completed); governed Knowledge display names wherever codes leaked (logging selector, requirement rows; no hard-coded mapping; identity/payload unchanged); timezone duplication corrected. New permanent `test:s3c-finalized-query` regression proves finalized → 0 leaderboard transport calls and active/unfinalized → 1 through a real QueryClient with the hook's real key/enablement, coupled to the actual hook source. S3b/S3a guard expectations updated to the corrected assembly (no logic change). Evidence: new regression PASS; S3c/S3b-CORR-001/S3a/S2b/membership/frontend/boundary guards PASS; api suites PASS (incl. S3c 10/10); API typecheck/build, root `tsc -b`, `vite build`, functions build clean; `git diff --check` clean; fresh Founder preview (hero + overlay + governed submission with live-state convergence) captured per the S3c record. **S3c is NOT marked COMPLETE or FOUNDER ACCEPTED; PR #38 remains unmerged; no deploy; S3d NOT STARTED.** Master Programme 1.91 → 1.92. |
| 1.91 | 2026-09-20 | **S3 IMPLEMENTATION IN PROGRESS; S3c IMPLEMENTED CANDIDATE / CORRECTED / TECHNICALLY REVALIDATED / READY FOR FOUNDER PREVIEW (TIIZI-S3C-FOUNDER-PREVIEW-PREP-001)** | **ITR-002 disposition B; local preview preparation passed (STOP BEFORE FOUNDER ACCEPTANCE / MERGE)** | Records TIIZI-S3C-LIVE-PROGRESS-ITR-002 disposition **B — APPROVABLE FOR FOUNDER PREVIEW WITH NON-BLOCKING OBSERVATIONS** and successful isolated local preparation from exact PR #38 candidate `85bbbcf`: Together authoritative 150/200 with contributors 80/50/20/0; Race live standard competition positions 1,1,3,null with 3 of 4 finished; Streak Africa/Nairobi governing day 2026-09-20 with current/best 2, days done 2 of 5 and Done/Pending today. Actual browser/runtime hard-refresh checks passed; no S3c console errors, raw UUID labels, malformed numeric rendering, S3d final results, Kudos, Support or media functionality observed. Preserves ITR-002 non-blocking observation: actual hook proved finalized → 0 leaderboard transport calls and active/unfinalized → 1; permanent repository regression deferred to S3c closure/acceptance unless Founder preview changes implementation. Presentation observations reserved for Founder review: canonical Activity Codes in Streak requirement rows and duplicated “time” in timezone copy. **S3c is NOT COMPLETE or FOUNDER ACCEPTED; PR #38 remains unmerged; no deploy; S3d NOT STARTED.** Master Programme 1.90 → 1.91. |
| 1.90 | 2026-09-19 | **S3 IMPLEMENTATION IN PROGRESS; S3c IMPLEMENTED CANDIDATE / CORRECTED / AWAITING TECHNICAL REVALIDATION (TIIZI-S3C-LIVE-PROGRESS-CORR-001)** | **Bounded correction slice on PR #38 (STOP BEFORE MERGE)** | Corrects the two ITR-001 blockers on branch `impl/s3c-live-progress-001` (base `origin/main` @ `af1d67e`; entry re-verified: no drift) with no scope expansion: (1) collective contributor reconciliation — `getChallengeContributors` is now member-level (contributionTotal/logsAccepted aggregate governed Derived Truth across ALL episodes; exposed participationId is the current episode per the display-episode rule; records stay attached to original episodes; canonical collective truth untouched); new A-class route regression `api/test/s3cLiveProgressCorr001.test.ts` proves join → 40 → withdraw → rejoin → 70 → GET contributors (total 110 / member 110 / share 1 / episode-2 current / episode-1 history intact) plus multi-member sum/share reconciliation and visibility/isolation; (2) finalized competitive boundary — `useCompetitiveLeaderboardV2` enables only for competitive + unfinalized (pure `competitiveLeaderboardEnabledForS3c`) and `V2CompetitiveProgress` returns null once finalized, so frozen final authority (still served by the route for future S3d, proven post-`finalizeChallenge`) is never displayed as S3c "live". Evidence: 4/4 CORR-001 tests PASS; S3c tests/guards green; S3a/S3b/S2b/membership/frontend/boundary guards green; API typecheck + full suite green; root `tsc -b` + `vite build` clean; functions build clean; `git diff --check` clean. **S3c is NOT marked COMPLETE; S3d NOT started**; no merge, no deploy, no Founder preview, no production mutation. Master Programme 1.89 → 1.90. |
| 1.89 | 2026-09-19 | **S3 IMPLEMENTATION IN PROGRESS; S3c IMPLEMENTED CANDIDATE / AWAITING TECHNICAL REVIEW (TIIZI-S3C-LIVE-PROGRESS-001)** | **Authorised implementation slice (STOP BEFORE MERGE)** | Records **S3 IMPLEMENTATION IN PROGRESS** and **S3c IMPLEMENTED CANDIDATE / AWAITING TECHNICAL REVIEW** on branch `impl/s3c-live-progress-001` (base `origin/main` @ `af1d67e`; entry re-verified: no drift; `ci` on base green). Implements the two bounded read-model projections from the Founder scope decisions: (A) collective contributor rollup — new `GET /v1/challenges/:id/contributors` (collective-only; contribution visibility, NOT a leaderboard: no position/rank/winner, shares of current total, overshoot preserved); (B) server-authoritative governing day — detail `governingToday` + `serverNow` (`dayInTimezone(now, timezone)`, injectable clock; browser date never authoritative). Corrects stale ranking-notation wording to standard competition ranking 1,1,3 (engine unchanged) and reconciles Run Again (NOT S3) + Kudos (NOT S3c) map references. Binds the V2 detail to per-type live truth (`V2ProgressSection`: Together shared-total/contributors, Race own-vs-target + server positions + N-of-M, Streak governing-today/current/best/day-states, live never terminally complete). Post-acceptance convergence through the extended canonical `invalidateV2ChallengeReads` contract (contributors + leaderboard families; per-user isolation). Evidence: new real-seam `api/test/s3cLiveProgress.test.ts` (6 tests: collective 2-member, competitive tie 1,1,3 + nulls, streak tz-boundary/partial/dup/miss/closed-day/never-terminal, contributors HTTP type gates) + new `test:s3c-live-progress` guards; S3a/S3b/S2b/membership/frontend/boundary guards green; API typecheck + full suite green; root `tsc -b` + `vite build` clean; functions build clean; `git diff --check` clean. **S3a/S3b remain COMPLETE / FOUNDER ACCEPTED / MERGED** (unchanged); **S3c is NOT marked COMPLETE; S3d NOT started**; no merge, no deploy, no Founder preview, no production mutation. Master Programme 1.88 → 1.89. |
| 1.88 | 2026-09-19 | **S3 IMPLEMENTATION IN PROGRESS; S3b COMPLETE / FOUNDER ACCEPTED (TIIZI-S3B-FOUNDER-ACCEPT-MERGE-001)** | **Founder acceptance + authorised merge (normal merge-commit path; no squash, no rebase, no force-push; no deploy)** | Records **S3b COMPLETE / FOUNDER ACCEPTED** on branch `impl/s3b-activity-application-001` (candidate `9ad0ef2`; PR #37 OPEN). **Founder acceptance evidence:** the existing Founder-created Challenge loaded successfully; activity logging succeeded through the normal UI; the UI displayed **"Recorded. 20 kilometres counted for this Challenge. Points awarded: 0 · Day: 2026-09-19"**; **"Log another"** was exercised and reset the form for a new activity; a normal refresh loaded the Challenge with no error; no Founder-observed error. The **0-point result is expected governed server behaviour** for that entry (20/500 = 4 % is below the 5 % minimum-effort ratio) and is **NOT a defect**. Retained technical/live evidence: the immutable Knowledge identity application-route correction (CORR-002) passed; the PostgreSQL DATE projection correction (CORR-003) passed; multiple accepted submissions were persisted without duplication; **CORR-002 and CORR-003 remain CLOSED**. S3 remains IMPLEMENTATION IN PROGRESS; S3a remains COMPLETE / FOUNDER ACCEPTED / MERGED; S3c/S3d NOT STARTED. Merge of PR #37 into canonical main is authorised under this entry via the repository's normal merge-commit path (accepted head `9ad0ef2` ancestor of the resulting main; the merge commit is recorded by the follow-up merged-state entry). No deployment; no production data mutation. Master Programme 1.87 → 1.88. [MERGED 2026-09-19: PR #37 merged by normal merge commit `99e2d4959cfafaab262563e941c5ffa7e3421709` of accepted head `d09b507ca035450a48dd99ca7f7e2de591bcc1f2` into `origin/main`; accepted head verified ancestor of main; post-merge `ci` green (api, api-image, functions, web).] |
| 1.87 | 2026-09-19 | **S3 IMPLEMENTATION IN PROGRESS; S3b IMPLEMENTED CANDIDATE / CORRECTED / FOUNDER REVALIDATION PASSED / AWAITING FOUNDER ACCEPTANCE (TIIZI-S3B-FOUNDER-REVALIDATION-RESUME-001)** | **Founder revalidation resumed and PASSED (STOP BEFORE MERGE; acceptance is a separate Founder disposition)** | Resumes the S3b Founder preview on branch `impl/s3b-activity-application-001` (candidate `0640108`; PR #37 OPEN, NOT merged) after the external port-9099 blocker was removed. Port 9099 was **free**; the S3b Auth/Firestore emulators were restored from the existing recovery set `/private/tmp/tiizi-s3b-emulator-recovery.dRMjBf` under the authorised config (project `demo-tiizi-s3b`). The Founder account `founder1@tiizi.local` was restored at its **same UID `Tgj1jC3FXyxOfqtdlhvelt69phoI`**, still linked to PostgreSQL member `7acce992-8450-4f6b-9431-9939fca56cad` (port 5435); no replacement identity was created. Existing product state was retained unchanged (Group `ae24cde5…` "test group1"; collective/Together Challenge `e13229fb…` active, window 2026-09-19..2026-10-02, Africa/Nairobi; configured Community Walk UUID `b946a8f8…`; participation `25ac9602…` active; the four historical rejected intents preserved). **Live Founder journey through the real UI (headless Chromium):** sign-in → same Challenge → logging surface → submit Community Walk **30 km** → **accepted** (`occurredDay 2026-09-19`, `pointsAwarded 6`, `duplicate false`; no `unknown_activity`, no `outside_challenge_window`; Community Walk shown without a raw UUID) → **Log another** (amount cleared, occurrence time reset, new intent key) → submit **40 km** → **accepted** (`pointsAwarded 8`, same day) → **refresh coherent**. Persisted after the journey: `challenge_activity_records = 3`, `member_activity_events = 3`, `activity_submission_intents = 7` (3 accepted + 4 rejected preserved), distinct client keys = 3, **no duplicate records**; derived truth `participation logs=3 / points=14 / cumulative=90`, `challenge collective_total=90` (goal not reached, not terminal). A preceding capture run also accepted a 20 km entry (`pointsAwarded 0`, below the 5% minimum-effort ratio) whose UI confirmation was not captured due to an over-eager reload; it is retained as committed evidence (**3 total accepted records, not 2**). **CORR-002 (identity) and CORR-003 (DATE) both work in the assembled product; no implementation correction was required by this task.** S3b is **FOUNDER REVALIDATION PASSED / AWAITING FOUNDER ACCEPTANCE** — **NOT marked COMPLETE and NOT FOUNDER ACCEPTED**; S3c/S3d NOT started; no merge, no deploy. S3a remains COMPLETE / FOUNDER ACCEPTED / MERGED. Master Programme 1.86 → 1.87. |
| 1.86 | 2026-09-19 | **S3 IMPLEMENTATION IN PROGRESS; S3b IMPLEMENTED CANDIDATE / CORRECTED / FOUNDER REVALIDATION REQUIRED (TIIZI-S3B-FOUNDER-PREVIEW-CORR-003)** | **Founder-preview blocking PostgreSQL DATE projection correction (STOP BEFORE MERGE)** | Corrects the DATE projection defect exposed by CORR-002 on branch `impl/s3b-activity-application-001` (base `origin/main` @ `e020d7f`; entering head `fca54bb`; PR #37 OPEN, NOT merged). A PostgreSQL `DATE` is a calendar day, never an instant; node-postgres hands it to Node as a JS `Date` at **server-local midnight**, so projecting it with `toISOString().slice(0,10)` shifted the stored day back on positive-offset hosts (host `Africa/Bujumbura` UTC+2: stored `2026-09-19` → `2026-09-18`). The accepted-path period gate in `applyChallengeActivity` therefore rejected a valid in-window log as `422 outside_challenge_window` (Founder Challenge `e13229fb…`, window 2026-09-19..2026-10-02, Community Walk good). Fix is representation-only (no schema/migration, no eligibility relaxation): reuse the canonical DATE-safe `toDayString` (v1.76, `api/src/challengeConfigs.ts`) in every affected occurrence-day DATE projection in the activity/Challenge path — `api/src/activityEvents.ts` `normalizeRow`; `api/src/challengeActivityApplication.ts` `normalizeRecord`; `api/src/derivedTruth.ts` `toDayOrNull` + replay normalization; `api/src/submissionIntents.ts` `normalizeSubmissionIntentRow` (also the idempotency payload binding, where the bad projection could make a legitimate retry look like a changed payload). Challenge-window validation, governing-day authority, `occurred_at` semantics, `occurred_tz` provenance, `STREAK_DAY_CLOSED`/no-grace, and participation-episode validation are unchanged. New regression `api/test/activityEventDateProjection.test.ts`: (a) driver-Date / UTC-midnight / string normalization across `2026-09-19`, `2026-09-30`, `2026-10-01`, `2026-12-31`, `2027-01-01`; (b) real application-path boundary (window 2026-09-19..2026-10-02, governing day 2026-09-19 → accepted, not `outside_challenge_window`). Proven to FAIL against pre-CORR-003 code under `TZ=Africa/Nairobi` (`expected '2026-09-18' to be '2026-09-19'`) and PASS under `TZ=UTC`, `TZ=Africa/Bujumbura`, `TZ=Africa/Nairobi`, `TZ=America/New_York`. CORR-002 identity regression + the two real-route suites 80 passed; API full suite 642 passed / 8 skipped / 0 failed; API typecheck + build clean; root `tsc -b` clean; `vite build` clean; functions build clean; S3b/S3a/S2b/membership/frontend/boundary guards PASS; `git diff --check` clean. **Live Founder revalidation could not be resumed:** the Klockit project's emulator (`firebase emulators:start --only auth --project demo-klockit-local`, cwd `/Volumes/PRODUCTION/Projects/klockit`, PID 41922) still exclusively owns port 9099, which Tiizi Auth requires by committed product configuration; no other project's process was touched; the S3b emulator recovery set `/private/tmp/tiizi-s3b-emulator-recovery.dRMjBf` preserves the Founder Auth UID `Tgj1jC3FXyxOfqtdlhvelt69phoI`. **S3b is NOT marked COMPLETE or FOUNDER ACCEPTED; S3c/S3d NOT started; no merge, no deploy.** S3a remains COMPLETE / FOUNDER ACCEPTED / MERGED. Master Programme 1.85 → 1.86. |
| 1.85 | 2026-09-19 | **S3 IMPLEMENTATION IN PROGRESS; S3b IMPLEMENTED CANDIDATE / CORRECTED / FOUNDER REVALIDATION REQUIRED (TIIZI-S3B-FOUNDER-PREVIEW-CORR-002)** | **Founder-preview blocking integration defect correction (STOP BEFORE MERGE)** | Corrects DEFECT-001 on branch `impl/s3b-activity-application-001` (base `origin/main` @ `e020d7f`; entering head `8d27476`; PR #37 OPEN, NOT merged): the V2 activity-application route (`POST /v1/challenges/:id/activity`) injected the quarantined exact-NAME Knowledge pin resolver (`createDbKnowledgeResolver` → `resolveKnowledgePinByName`) while V2 establishment (`validateChallengeDefinition` / PF-01-CORR-001), the Challenge read model and the S3b client all use immutable identity (Knowledge UUID / governed Activity Code). A governed-UI-created Challenge therefore displayed its configured activity correctly but `applyChallengeActivity` failed pin resolution and rejected every valid log as `422 unknown_activity` / `ACTIVITY_NOT_CONFIGURED` (Founder Challenge `e13229fb…`, configured activity Community Walk UUID `b946a8f8…`). Fix: new `createDbKnowledgeIdentityFirstResolver` (`api/src/knowledgePins.ts`) — identity-first (UUID / Activity Code) with a narrowly bounded exact-NAME fallback for historical pre-PF-01 name-pinned configs (exact published name only; no fuzzy/partial matching; unknown keys still fail closed); wired into `api/src/challengeActivityRoutes.ts`. The fallback was required by repository evidence: the existing real-route tests `challengeActivityApplication.test.ts` and `ebc02SubmissionAcceptanceTrace.test.ts` assert the V2 activity-application contract still applies legacy name-pinned configs. Establishment is unchanged and still rejects display names as identity, so the fallback can never create a name-pinned config. New real-route regression `api/test/s3bActivityLoggingIntegration.test.ts` (5 cases: Activity-Code, UUID, unknown identity fail-closed, non-configured identity rejected, legacy name fallback) PASS; the two previously failing real-route tests PASS; API full suite 637 passed / 8 skipped / 0 failed; API typecheck + build clean; root `tsc -b` clean; `vite build` clean; functions build clean; S3b/S3a/S2b/membership/frontend/boundary guards PASS; `git diff --check` clean. **Two blockers recorded (not silently fixed):** (1) FOUNDER-REVALIDATION BLOCKER — a distinct, pre-existing timezone-unsafe DATE→string projection in `api/src/activityEvents.ts` `normalizeRow` (`row.occurred_day.toISOString().slice(0,10)`) mis-projects a Postgres DATE at local midnight to the previous UTC day on positive-offset hosts (host TZ CAT/UTC+2: DATE `2026-09-19` → `2026-09-18`), so the accepted-path period gate rejects a valid in-window log as `422 outside_challenge_window`; same class already corrected for Challenge dates in v1.76 (`toDayString`), NOT proven by DEFECT-001 and outside this bounded correction — a separate authorised correction is recommended; (2) EXTERNAL BLOCKER — during revalidation the Klockit project's tooling (`firebase emulators:start --only auth --project demo-klockit-local`, cwd `/Volumes/PRODUCTION/Projects/klockit`, PID 41922) claimed port 9099 and terminated the S3b Auth/Firestore emulator; no other project's process was touched; the S3b emulator exported a clean recovery set to `/private/tmp/tiizi-s3b-emulator-recovery.dRMjBf` preserving the Founder Auth UID `Tgj1jC3FXyxOfqtdlhvelt69phoI`. **S3b is NOT marked COMPLETE or FOUNDER ACCEPTED; S3c/S3d NOT started; no merge, no deploy.** S3a remains COMPLETE / FOUNDER ACCEPTED / MERGED. Master Programme 1.84 → 1.85. |
| 1.84 | 2026-09-19 | **S3 IMPLEMENTATION IN PROGRESS; S3b IMPLEMENTED CANDIDATE / CORRECTED / AWAITING TECHNICAL REVALIDATION (TIIZI-S3B-ACTIVITY-APPLICATION-CORR-001)** | **Authorised correction slice (STOP BEFORE MERGE)** | Corrects the three blocking ITR-001 defects on branch `impl/s3b-activity-application-001` (base `origin/main` @ `e020d7f`; candidate `75a3024`; PR #37 OPEN, NOT merged) with NO API/schema/migration/domain change: (1) S3b submission omits client-derived `occurred_day` via the bounded `buildS3bActivityPayload` (shared builder default preserved for frozen surfaces; `occurred_tz` kept as provenance; governing day displayed from the authoritative server result; server mismatch validation untouched); (2) explicit submission-intent/key model (`loggingIntent.ts`: same facts → same key, any fact change → new key before submission; governed-rejection correction and post-acceptance changes mint new keys; server `isSameSubmissionPayload` binding untouched); (3) accepted-result rendering separated from new-entry gating (`loggingSectionStateFor`: an authoritative accepted result stays visible across an active→ended refetch while ended/closed reads forbid new submissions; no progress preserved or manufactured). Bounded UX: UUID-free selector labels with the resolved Knowledge name primary, honest zero-activity empty state, Log-another re-establishes a cleared new-entry intent, `no_participation_episode` copy refers to the participation period, `idempotency_key_conflict` copy directs to a new entry without claiming non-recording. Evidence: new behavioral `test:s3b-activity-application-corr-001` guards PASS plus updated `test:s3b-activity-logging` guards PASS; S3a/S2b/membership/frontend/boundary guards green; API typecheck clean + full suite 632 passed / 8 skipped; root `tsc -b` + `vite build` clean; functions build clean; `git diff --check` clean. **S3b is NOT marked COMPLETE; S3c/S3d NOT started; no merge, no deploy, no Founder preview.** S3a remains COMPLETE / FOUNDER ACCEPTED / MERGED. Master Programme 1.83 → 1.84. |
| 1.83 | 2026-09-18 | **S3 IMPLEMENTATION IN PROGRESS; S3b IMPLEMENTED CANDIDATE / AWAITING TECHNICAL REVIEW (TIIZI-S3B-ACTIVITY-APPLICATION-001)** | **Authorised implementation slice (STOP BEFORE MERGE)** | Records **S3 IMPLEMENTATION IN PROGRESS** and **S3b IMPLEMENTED CANDIDATE / AWAITING TECHNICAL REVIEW** on branch `impl/s3b-activity-application-001` (base `origin/main` @ `e020d7f`; entry re-verified: no drift from the S3 charter §2B authority map; `ci` on base green). Binds the existing governed activity-application authority (`applyChallengeActivity`, governing config version at acceptance, server-side scoring, `client_key` idempotency replay, durable rejected intents, streak day-closed/no-grace) over the existing seam (`POST /v1/challenges/:id/activity`; what-counts presentation from the detail governing config — no new API, no read-model change, no second scoring engine, no client-derived score/progress). The existing V2 Challenge detail route (`V2CreatedChallengeScreen` — no parallel detail system) gains the governed logging section for joined participants on active Challenges only (eligible choices derived from `config.activities` with locked unit/kind/variant; member supplies amount + when; `occurred_day`/`occurred_tz` auto-derived; one `client_key` per intentional entry reused across retries): accepted entries render the server result honestly (recorded value/unit, awarded points, day, duplicate notice; role=status) and governed rejections render human-readable copy with the server code preserved and retryable=false (incl. `streak_day_closed` with no late logging); infrastructure failures (`503`/`group_authority_unavailable`/network) render distinctly as retryable with a same-key Retry affordance, while governed rejections offer a fresh entry (new key — resubmitting changed facts under a rejected key would conflict). Post-acceptance truth is refetch-only (canonical + legacy cache families invalidated together via the established `challengeQueryKeys` contract — no repeat of the S2-G/S2b stale-cache defect; per-user isolation preserved; new `test:s3b-activity-logging` guards green alongside the S3a/S2b guards). Slice record: `docs/experience/TIIZI-S3B-ACTIVITY-APPLICATION.md`. **S2 remains COMPLETE / FOUNDER ACCEPTED / MERGED** (unchanged); **S3a remains COMPLETE / FOUNDER ACCEPTED / MERGED** (unchanged); **S3b is NOT marked COMPLETE; S3c/S3d NOT started**. Contribution/support boundary holds (RECON-001 disposition A: financial contributions are not activities — payload allowlist-closed, runtime excess-key drop proven; no donation/support/cause UI, fields, or infra). Custom Duration / Group + Challenge cover media NOT implemented; PF-05 NOT resurrected; PF-06 NOT begun; EBC-05 unmerged/reference-only; V1 FROZEN; no migration; no deployment; no production data mutation. Master Programme 1.82 → 1.83. |
