...
| ------- | ---------- | ------ | -------- | ------ |
| 1.96 | 2026-09-21 | **S3 IMPLEMENTATION IN PROGRESS; S3c stays COMPLETE / FOUNDER ACCEPTED / MERGED — bounded post-S3c presentation cleanup (TIIZI-S3C-DETAIL-CLEANUP-001)** | **Backlog-style post-acceptance experience cleanup (candidate; PR #39; NOT merged; no deploy, no S3d)** | Records a bounded presentation cleanup identified during continued Founder visual review **after** S3c acceptance/merge: the standalone "What counts" card is removed from the joined Challenge detail for Together/Race/Streak, because the participant surfaces already express it (Group Progress + shared target + contributors; Race Progress + target + Race Standings; Today's Daily Consistency + requirements/streak/days) and the governed Log Activity overlay exposes governed activity names/units at the point of logging. The Streak duration rule is preserved once inside Today's Daily Consistency from the governed `requiredConsecutiveDays`. Presentation only — no canonical Challenge truth, contract, read projection, logging eligibility, activity identity, target, unit, progress calculation, race ranking, streak or participation semantics changed; Log Activity overlay and Leave Challenge flow untouched; no migration. **S3c is NOT reopened and its acceptance is unchanged (COMPLETE / FOUNDER ACCEPTED / MERGED); S3a/S3b remain COMPLETE / FOUNDER ACCEPTED / MERGED; S3 remains IMPLEMENTATION IN PROGRESS; S3d NOT STARTED.** Record: `docs/experience/TIIZI-S3C-LIVE-PROGRESS.md` §16. Master Programme 1.95 → 1.96. |
